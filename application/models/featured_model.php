...
//		return array($name, $image, $latest);
	...
	function featured_venue_info()
	{
		$f_id = $this->get_featured('venue');
		$f_venues = array();
		//		error_log("Length of the f_id array from featured_venue_info: " . count($f_id));
	
		for($i=0; $i<count($f_id); $i++)
		{
			array_push($f_venues, array());
			
			// fetch venue info
			$result = $this->db->get_where('venues', array('venue_id' => $f_id[$i]));
			if($result->num_rows() == 0)
				return 'fail';
			foreach($result->result() as $row)
			{
				$f_venues[$i][] = $row->venue_id;
				$f_venues[$i][] = $row->image;
				$f_venues[$i][] = $row->city . ", " . $row->state;
			}
			
		}
	
		/*
		foreach($f_venues as $key => $value){
			foreach($value as $innervalue){
				error_log("FVENUE VALUE OF $key and $value and $innervalue");
			}
		}
		*/
